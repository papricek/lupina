...
module Lupina
  class EdcGenerator
    ...
    # consumption_profile: { weekday: [24 floats], weekend: [24 floats] }
    #   Custom hourly profile (0.0–1.0) generated by LLM. Takes priority.
    # consumption_pattern: symbol fallback
    #   :afternoon_weekend, :minimal, :industrial_lunch_break, :early_shift, :residential, :flat
    ...
    def pattern_value_at(mid, weekday)
      case @consumption_pattern
      when :afternoon_weekend
        weekday ? afternoon_weekend_wd(mid) : afternoon_weekend_we(mid)
      when :minimal
        minimal_consumption(mid)
      when :industrial_lunch_break
        weekday ? industrial_lunch_wd(mid) : industrial_lunch_we(mid)
      when :early_shift
        weekday ? early_shift_wd(mid) : early_shift_we(mid)
      when :residential
        residential_consumption(mid, weekday)
      else
        0.5
      end
    end
